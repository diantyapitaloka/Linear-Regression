...
- Linear regression strictly requires numerical inputs, so categorical data like "Neighborhood" and "House Style" must be converted into numbers. All we typically use One-Hot Encoding to create "dummy variables" (0 or 1) for each category to represent these qualitative traits. It is crucial to drop one dummy column (the reference category) to avoid the "Dummy Variable Trap," which a causes perfect multicollinearity.
