...
- While MAE gives equal weight to all errors, RMSE squares the errors before averaging them, making it heavily penalize larger mistakes. Knowing whether your business case prioritizes avoiding huge single errors (use RMSE) or understanding average typical error (use MAE) dictates which metric to track.
- When you want the feature-selection power of Lasso (L1) and the stability of Ridge (L2), ElasticNet combines both penalties into a single loss function. It is particularly effective when dealing with highly correlated features where Lasso tends to randomly pick just one.
- While standard $R^2$ automatically increases whenever you add new features (even completely useless ones), Adjusted $R^2$ penalizes the score for adding predictors that don’t improve the model's predictive power. It provides a much more honest evaluation when building Multiple Linear Regression models.
